const debug = require('debug')('@essential-projects/meta:listDirectories');
const fs = require('fs');
...
module.exports = dir => {
  ...
  const essentialProjectsGlobalPlugins = fs.readdirSync(essentialProjectsDir).filter(file => {
    console.log(path.join(essentialProjectsDir, file));
    return (
  ...
  const essentialProjectsGlobalPluginsFullPath = essentialProjectsGlobalPlugins.map(r => {
    return path.resolve(essentialProjectsDir, r);
  });
