...
const regex = /^(meta-|@essential-projects\/meta-).*/;
...
module.exports = dir => {
  ...
  try {
    localPlugins = fs.readdirSync(localdir).filter(file => {
      ...
      if (file.startsWith('@')) {
        ...
        const localPluginPath = path.join(localdir, file);
        const localScopedPlugins = fs.readdirSync(localPluginPath).filter(scopedFile => {
          return (
            (fs.statSync(path.join(localPluginPath, scopedFile)).isDirectory() ||
              fs.lstatSync(path.join(localPluginPath, scopedFile)).isSymbolicLink()) &&
            regex.test(scopedFile)
          );
        });

        return localScopedPlugins.map((plugin) => {
          return `${file}/${scopedFile}`;
        });
  ...
  const globalPlugins = fs.readdirSync(dir).filter(file => {
    if (file.startsWith('@')) {
      ...
      const globalPluginPath = path.join(dir, file);
      const globalScopedPlugins = fs.readdirSync(globalPluginPath).filter(scopedFile => {
        console.log(path.join(globalPluginPath, scopedFile));
        return (
          (fs.statSync(path.join(globalPluginPath, scopedFile)).isDirectory() ||
            fs.lstatSync(path.join(globalPluginPath, scopedFile)).isSymbolicLink()) &&
          regex.test(scopedFile)
        );
      });
      
      return globalScopedPlugins.map((plugin) => {
        return `${file}/${scopedFile}`;
      });
